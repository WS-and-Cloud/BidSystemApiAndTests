namespace BidSystem.Tests
{
    ...
    public static class TestingEngine
    {
        ...
        public static HttpResponseMessage GetOffers(string userSessionToken, string actionName, int? offerId)
        {
            var endPoint = "/api/offers/" + actionName;
            if (offerId != null)
            {
                endPoint += '/';
                endPoint += offerId.ToString();
            }

            HttpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + userSessionToken);
